// st80-2026 — LinuxBridge.cpp
// Copyright (c) 2026 Aaron Wohl. MIT License.
//
// Implements Bridge.h on top of LinuxHal + Interpreter +
// PosixFileSystem. Structurally identical to AppleBridge.cpp; the
// only difference is which IHal subclass we own. The frontend (SDL2
// in app/linux) drives the same C API that the Swift frontend drives
// on Apple, so the two stay in lockstep.
...
// word, not per logical event, so a multi-word event doesn't strand
// trailing words in the queue. Match that pattern.
...
void expandDirtyRectToRGBA(Runtime &rt,
                           const st80::LinuxHal::DirtyRect &d,
                           int displayBits) {

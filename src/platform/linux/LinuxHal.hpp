// st80-2026 — LinuxHal.hpp
// Copyright (c) 2026 Aaron Wohl. MIT License.
//
// IHal implementation for Linux. Structurally identical to AppleHal
// — owns the RGBA8 staging buffer, dirty-rect tracking, a thread-safe
// EventQueue of 16-bit Blue Book input words, and the `signal_at`
// scheduling slot. No GUI library dependency lives inside the HAL
// itself: the frontend (SDL2 today; X11 or Wayland later) reads
// `displayPixels()` and pushes platform events through the C bridge
// in `src/include/Bridge.h`, which turns them into pre-encoded Blue
// Book input words.
...
class LinuxHal : public IHal {
 public:
    LinuxHal();
    ~LinuxHal() override = default;

// st80-2026 — WindowsBridge.cpp
// Copyright (c) 2026 Aaron Wohl. MIT License.
//
// Implements Bridge.h on top of WindowsHal + Interpreter +
// WindowsFileSystem. Structurally identical to LinuxBridge.cpp;
// the only differences are (1) the HAL subclass and (2) the
// image-path splitter accepts both `\` and `/`. The frontend
// (app/windows) drives the same C API that the Linux SDL frontend
// and the Swift/Metal frontend on Apple drive, so the three stay
// in lockstep.
...
struct Runtime {
    st80::WindowsHal hal;
    std::unique_ptr<st80::WindowsFileSystem> fs;
...
void expandDirtyRectToRGBA(Runtime &rt,
                           const st80::WindowsHal::DirtyRect &d,
                           int displayBits) {
...
extern "C" bool st80_init(const char *imagePath) {
    ...
    rt->fs = std::make_unique<st80::WindowsFileSystem>(dir);
...
extern "C" void st80_post_key_up(int /*charCode*/, uint32_t /*modifiers*/) {
    // Decoded keyboard — no separate up event. Symmetry with Apple/Linux.
}

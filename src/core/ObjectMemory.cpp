// st80-2026 — ObjectMemory.cpp
//
// Ported from dbanay/Smalltalk @ ab6ab55:src/objmemory.cpp
...
// === Byte-swap helpers for endian-aware loading ============================
// The Xerox v2 image is big-endian. Modern hosts (macOS, Linux, Windows) are
// little-endian. When the loader detects a mismatch it swaps on the fly so we
// never require an offline preprocessing step. See docs/image-preprocessing.md.
#if defined(__GNUC__) || defined(__clang__)
static inline std::uint16_t bswap16(std::uint16_t v) { return __builtin_bswap16(v); }
static inline std::uint32_t bswap32(std::uint32_t v) { return __builtin_bswap32(v); }
#else
static inline std::uint16_t bswap16(std::uint16_t v) {
    return static_cast<std::uint16_t>((v << 8) | (v >> 8));
}
static inline std::uint32_t bswap32(std::uint32_t v) {
    return ((v & 0xff000000u) >> 24) | ((v & 0x00ff0000u) >> 8)
         | ((v & 0x0000ff00u) << 8)  | ((v & 0x000000ffu) << 24);
}
#endif

// Image-specific class OOP for WordArray (a class that holds 16-bit words as
// data, not pointers or bytes). Not part of the Blue Book well-known OOPs;
// value matches the Xerox v2 image (per dbanay misc/imageswapper.c:28).
// Only consulted during endian swap to decide whether to swap body words.
static constexpr int ClassWordArrayPointer = 0xA72;  // 2674
...
bool ObjectMemory::loadObjectTable(IFileSystem *fileSystem, int fd)
{
    ...
    // Endian detection. If the host-native read of objectTableLength is
    // implausibly large for the file size, the image was written in the
    // opposite byte order — byte-swap the header and remember to swap every
    // subsequent uint16 read.
    auto plausibleOTLen = [fileSize](std::int32_t len) {
        return len > 0 && len <= fileSize / 2;
    };
    if (!plausibleOTLen(objectTableLength)) {
        std::int32_t swapped = static_cast<std::int32_t>(
            bswap32(static_cast<std::uint32_t>(objectTableLength)));
        if (!plausibleOTLen(swapped))
            return false;  // Neither orientation makes sense — corrupt image.
        objectTableLength = swapped;
        swapOnLoad = true;
    } else {
        swapOnLoad = false;
    }
    ...
    for(int objectPointer = 0; objectPointer < objectTableLength; objectPointer+=2)
    {
        ...
        if (fileSystem->read(fd, (char *)&words, sizeof(words)) != sizeof(words))
            return false;
        if (swapOnLoad) {
            words[0] = bswap16(words[0]);
            words[1] = bswap16(words[1]);
        }
...
bool ObjectMemory::loadObjects(IFileSystem *fileSystem, int fd)
{
    ...
    for(int objectPointer = 2; objectPointer < ObjectTableSize; objectPointer += 2)
    {
        ...
        if (swapOnLoad) objectSize = bswap16(objectSize);
        ...
        if (swapOnLoad) classBits = bswap16(classBits);
        ...
        for(int wordIndex = 0; wordIndex < objectSize-HeaderSize; wordIndex++)
        {
            ...
            // use heap chunk; post-processed below per class if swap is on
            storeWord_ofObject_withValue(wordIndex, objectPointer, word);
        }
        if (swapOnLoad)
            swapObjectBodyBytes(objectPointer, classBits);
...
// Per-class byte-swap of an object's body after raw bytes have been loaded
// from a foreign-endian image. Mirrors the class dispatch in dbanay's
// misc/imageswapper.c so that ports load unswapped Xerox images (BE) on
// little-endian hosts (and vice versa).
void ObjectMemory::swapObjectBodyBytes(int objectPointer, int classPointer)
{
    const int bodyWords = fetchWordLengthOf(objectPointer);
    const bool hasPointers = pointerBitOf(objectPointer) != 0;

    if (classPointer == ClassCompiledMethod) {
        if (bodyWords < 1) return;
        // Body word 0 is the method header. Swap it, use it to compute
        // the literal count, then swap each literal. Bytecodes (the rest
        // of the body) are per-byte and must stay raw.
        std::uint16_t header = bswap16(static_cast<std::uint16_t>(
            fetchWord_ofObject(0, objectPointer)));
        storeWord_ofObject_withValue(0, objectPointer, header);
        const int literalCount = (header & 0x7E) >> 1;
        for (int i = 1; i <= literalCount && i < bodyWords; i++) {
            std::uint16_t w = bswap16(static_cast<std::uint16_t>(
                fetchWord_ofObject(i, objectPointer)));
            storeWord_ofObject_withValue(i, objectPointer, w);
        }
    } else if (classPointer == ClassFloatPointer) {
        // Float is 4 bytes = 2 words. A BE→LE (or LE→BE) byte reversal
        // flips the whole 4-byte sequence: new w0 = swap16(old w1),
        // new w1 = swap16(old w0).
        if (bodyWords < 2) return;
        std::uint16_t w0 = static_cast<std::uint16_t>(fetchWord_ofObject(0, objectPointer));
        std::uint16_t w1 = static_cast<std::uint16_t>(fetchWord_ofObject(1, objectPointer));
        storeWord_ofObject_withValue(0, objectPointer, bswap16(w1));
        storeWord_ofObject_withValue(1, objectPointer, bswap16(w0));
    } else if (hasPointers ||
               classPointer == ClassDisplayBitmapPointer ||
               classPointer == ClassWordArrayPointer) {
        // Pointer-type objects, DisplayBitmap, and WordArray: swap every
        // body word.
    ...
    // else: pure byte-type object (String, Symbol, ByteArray, …). The body
    // is per-byte; its raw file bytes already land at the right memory
    // addresses for IFileSystem::read into a uint16[] and RealWordMemory's
    // byteNumber=0-is-first-byte accessor convention.
}

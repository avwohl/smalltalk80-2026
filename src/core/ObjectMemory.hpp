// st80-2026 — ObjectMemory.hpp
//
// Ported from dbanay/Smalltalk @ ab6ab55:src/objmemory.h
...
//
// Blue Book Object Memory (G&R ch. 27). Object Table in segment 15,
// heap in segments 0..14. Hybrid ref-count + mark/sweep GC, selected
// via the GC_MARK_SWEEP and GC_REF_COUNT macros below. Keeping
// dbanay's class shape for bug-for-bug fidelity; a future cleanup
// may drop GC_REF_COUNT per docs/plan.md.
...
// Ref counting. Disabled per docs/plan.md (pure mark/sweep policy —
// a 1 MB heap marks in microseconds so the per-store RC overhead
// isn't worth the save). dbanay's code keeps both paths compiled; we
// leave `#define GC_REF_COUNT` commented so future bisection is easy.
// #define GC_REF_COUNT
...
public:
#ifdef GC_MARK_SWEEP
    ObjectMemory(IHal *halInterface, IGCNotification *notification = 0);
#else
    ObjectMemory(IHal *halInterface);
#endif
...
private:
    ...
    // Post-load byte-swap for a single object's body when the image
    // was in foreign endianness. Swap policy depends on class (per
    // dbanay's imageswapper.c).
    void swapObjectBodyBytes(int objectPointer, int classPointer);
    ...
    // Set by loadObjectTable when it detects the image was written in
    // foreign byte order (typically: big-endian Xerox image on a
    // little-endian host, or vice versa). Used by loadObjects to
    // decide whether to byte-swap incoming size/class/body words.
    bool swapOnLoad = false;
};

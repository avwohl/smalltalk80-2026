// st80-2026 — Interpreter.hpp
//
// Ported from dbanay/Smalltalk @ ab6ab55:src/interpreter.h
...
//
// Blue Book interpreter (G&R ch. 28) with primitives (ch. 29).
// Switch-dispatched. An IMethodExecutor seam for a later JIT
// (Phase 6, docs/jit-plan.md) will wrap this class rather than
// replace it.
...
public:
    ...
    Interpreter(IHal *halInterface, IFileSystem *fileSystemInterface);

    // Interpreter inherits IGCNotification (under GC_MARK_SWEEP) so
    // deletion through a base pointer is possible. dbanay's original
    // lacks a virtual dtor; added here to keep `delete base_ptr`
    // well-defined.

// st80-2026 — Interpreter.cpp
//
// Ported from dbanay/Smalltalk @ ab6ab55:src/interpreter.cpp
...
Interpreter::Interpreter(IHal *halInterface, IFileSystem *fileSystemInterface) :
#ifdef GC_MARK_SWEEP
...
{
    // Width/height kept in the signature for API stability with
    // dbanay's interpreter.h; we derive them from currentDisplay.
    (void)width;
    (void)height;
...
{
    (void)destForm;  // kept for interface fidelity with dbanay; we
                     // pull the active form from currentDisplay.
...
void Interpreter::initializeMethodCache()
{
   ...
   */
    for(std::size_t i = 0; i < sizeof(methodCache)/sizeof(methodCache[0]); i++)
